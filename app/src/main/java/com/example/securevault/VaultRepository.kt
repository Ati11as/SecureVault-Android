...
class VaultRepository(private val context: Context, private val crypto: CryptoManager) {
    ...
    fun list(): List<VaultFile> = dir.listFiles()
        ?.asSequence()
        ?.filter { it.isFile && it.extension == "svault" }
        ?.mapNotNull { f ->
            val m = meta(f)
            ...
            VaultFile(
                ...
                lines.getOrElse(0) { "file" },
                lines.getOrElse(2) { "0" }.toLongOrNull() ?: 0,
                lines.getOrElse(1) { "application/octet-stream" },
                f
            )
        }
    ...
    fun decryptTo(v: VaultFile, target: File) {
        v.file.inputStream().use { input ->
            target.outputStream().use { out -> crypto.decrypt(input, out) }
        }
    }
